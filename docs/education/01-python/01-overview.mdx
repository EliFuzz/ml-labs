...
<Tabs queryString="primary">
    <TabItem value="internals" label="Internals">
        ...
        **Internal working of Python**
        ...
        ```mermaid
        graph LR
            CodeEditor(Code Editor) -->|1.Write Code| SourceFile("Source File (.py)")
            SourceFile -->|2.Save File| Compiler(Compiler)

            subgraph PythonInterpreter [Python Interpreter]
                Compiler -->|3.Compile to Bytecode| Bytecode("Bytecode (.pyc)")
                Bytecode -->|4.Execute| PVM("PVM")
            end
            PVM -->|6.Machine Code| RunningProgram("Running Program")
        ```
